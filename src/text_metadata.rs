...
//!  let decoder = rusty_png::Decoder::new(
...
//!
//!  ```
//!  # use rusty_png::text_metadata::{ITXtChunk, ZTXtChunk};
//!  # use std::env;
//!  # use std::fs::File;
//!  # use std::io::BufWriter;
//!  # use std::iter::FromIterator;
//!  # use std::path::PathBuf;
//!  # // Write to the temp dir, not "target/": in a workspace the target dir
//!  # // lives at the workspace root, so a crate-relative "target/" does not exist.
//!  # let _ = PathBuf::new();
//!  # let file = File::create(env::temp_dir().join("rusty_png_text_chunk.png")).unwrap();
