...
where
    F: Fn(PathBuf) -> Result<u32, rusty_png::DecodingError>,
{
...
fn render_images() {
    process_images("results.txt", &TEST_SUITES, |path| {
        let mut decoder = rusty_png::Decoder::new(File::open(path)?);
        decoder.set_transformations(rusty_png::Transformations::normalize_to_color8());
...
fn render_images_identity() {
    process_images("results_identity.txt", &TEST_SUITES, |path| {
        let decoder = rusty_png::Decoder::new(File::open(&path)?);
        let mut reader = decoder.read_info()?;
        let mut img_data = vec![0; reader.output_buffer_size()];
        let info = reader.next_frame(&mut img_data)?;
        let bits =
...
fn render_images_alpha() {
    process_images("results_alpha.txt", &TEST_SUITES, |path| {
        let mut decoder = rusty_png::Decoder::new(File::open(&path)?);
        decoder.set_transformations(rusty_png::Transformations::ALPHA);
        let mut reader = decoder.read_info()?;
        let mut img_data = vec![0; reader.output_buffer_size()];
        let info = reader.next_frame(&mut img_data)?;
        let bits =
